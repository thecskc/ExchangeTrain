...
class CoachConnectionRequests extends react__WEBPACK_IMPORTED_MODULE_0__["Component"] {
  ...
  handleResumeClick(event, resume) {
    event.preventDefault();
    console.log(resume);
    next_router__WEBPACK_IMPORTED_MODULE_5___default.a.push(resume); //window.location.href=resume;
    // try {
    //     Router.replace(resume).catch(function(err){
    //         console.log(err);
    //     });
  ...
  render() {
    if (this.state.loaded) {
      return __jsx(react__WEBPACK_IMPORTED_MODULE_0___default.a.Fragment, null, this.state.cards);
    } else {
      return __jsx("br", {
        __source: {
          fileName: _jsxFileName,
...
/* harmony import */ var _material_ui_core_DialogTitle__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(/*! @material-ui/core/DialogTitle */ "@material-ui/core/DialogTitle");
/* harmony import */ var _material_ui_core_DialogTitle__WEBPACK_IMPORTED_MODULE_10___default = /*#__PURE__*/__webpack_require__.n(_material_ui_core_DialogTitle__WEBPACK_IMPORTED_MODULE_10__);
/* harmony import */ var nextjs_redirect__WEBPACK_IMPORTED_MODULE_11__ = __webpack_require__(/*! nextjs-redirect */ "nextjs-redirect");
/* harmony import */ var nextjs_redirect__WEBPACK_IMPORTED_MODULE_11___default = /*#__PURE__*/__webpack_require__.n(nextjs_redirect__WEBPACK_IMPORTED_MODULE_11__);
...
class UserConnectionRequests extends react__WEBPACK_IMPORTED_MODULE_0__["Component"] {
  ...
  handleResumeClick(event, resume) {
    event.preventDefault();
    console.log(resume);
    next_router__WEBPACK_IMPORTED_MODULE_2___default.a.push(resume); //window.location.href=resume;
    // try {
  ...
  handleClick(event, status, fullDoc) {
    ...
    if (status === "USER_CALL_REQUEST") {//do Nothing
      ...
      const fb = _components_firebase__WEBPACK_IMPORTED_MODULE_1__["default"].firestore().collection("Connections").doc(userID + "_" + coachID).set({
        "status": "ISA_APPROVED"
      }, {
        merge: true
      });
  ...
  async getDoc(coachesList) {
    for (let i = 0; i < coachesList.length; i++) {
      const doc = await _components_firebase__WEBPACK_IMPORTED_MODULE_1__["default"].firestore().collection("Profiles").doc(coachesList[i].coach).get();
  ...
  componentDidMount() {
    let coachesList = [];
    const callReqRef = _components_firebase__WEBPACK_IMPORTED_MODULE_1__["default"].firestore().collection("Connections").where("user", "==", this.props.user.uid).get().then(snapShot => {
      snapShot.forEach(doc => {
        const obj = {
          "coach": doc.data().coach,
          "user": doc.data().user,
